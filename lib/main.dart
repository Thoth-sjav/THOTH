...
// ─── Background Service callback (corre no isolate separado) ──────────────
@pragma('vm:entry-point')
void onBackgroundServiceStart(ServiceInstance service) async {
  DartPluginRegistrant.ensureInitialized();
  // Manter o serviço vivo; enviar heartbeat a cada segundo
...
Future<void> main() async {
  ...
  // Capturar QUALQUER erro de build e mostrar no ecrã em vez de cinzento
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              const Text('ERRO', style: TextStyle(color: Colors.red, fontSize: 22, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              Text(details.exceptionAsString(), style: const TextStyle(color: Colors.white, fontSize: 13)),
              const SizedBox(height: 12),
  ...
  // Inicializar Foreground Service (só Android/iOS, não desktop/web)
...
class StreakInfo {
  ...
  static StreakInfo calcularComFreeze(List<SessaoConcluida> sessoes, List<String> freezeDias) {
    ...
    for (final s in sessoes) {
      final d = s.dataConclusao;
      diasEstudo.add('${d.year}-${d.month.toString().padLeft(2,"0")}-${d.day.toString().padLeft(2,"0")}');
    }
...
class _PomodoroAppState extends State<PomodoroApp>
 ...
 List<Tarefa> tarefas = [];
  ...
  Future<void> _carregarDados() async {
    // Timeout de segurança: mesmo que algo bloqueie, a app arranca ao fim de 20s
  ...
  /// e em memória local. Quando regressa: calcula os segundos passados.
  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    // 'hidden' é emitido no Android/iOS quando o ecrã desliga ou a app é
    // ocultada — tratamos igual a 'paused' para não perder a referência.
    ...
    if (isBackground) {
      ...
      if (estadoApp == EstadoApp.cronometro && !pausado) {
        // Guardar referência local imediatamente (sem esperar pelo Firestore)
        _timerReferencia = DateTime.now();
        _segundosNaReferencia = segundosRestantes;
        // Persistir também no Firestore para sobreviver a reinícios do processo
        _guardarEstadoTimer();
      }
    }
    ...
    if (state == AppLifecycleState.resumed) {
      if (estadoApp == EstadoApp.cronometro && !pausado) {
        // 1.ª tentativa: referência em memória (instantânea, sem rede)
        if (_timerReferencia != null && _segundosNaReferencia != null) {
          final elapsed = DateTime.now().difference(_timerReferencia!).inSeconds;
          final novosSegundos = (_segundosNaReferencia! - elapsed).clamp(0, 999999);
          if (mounted) {
          ...
          // 2.ª tentativa: Firestore (processo foi recriado, memória perdida)
  ...
  void iniciarTarefa(Tarefa t, {bool retomar = false}) {
    _timer?.cancel();
    // Arrancar Foreground Service para manter o processo vivo em background
    if (!kIsWeb && (Platform.isAndroid || Platform.isIOS)) {
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 13,
                      color: escuro ? Colors.white54 : Colors.black45,
                    ),
                    textAlign: TextAlign.center,
                  ),
                ),
        ...
        body: SafeArea(
          child: SingleChildScrollView(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('ERRO NO BUILD', style: TextStyle(color: Colors.red, fontSize: 20, fontWeight: FontWeight.bold)),
                const SizedBox(height: 16),
                Text(e.toString(), style: const TextStyle(color: Colors.white, fontSize: 13)),
                const SizedBox(height: 16),
                Text(stack.toString(), style: const TextStyle(color: Colors.white54, fontSize: 11)),
              ],
            ),
          ),
        ),
