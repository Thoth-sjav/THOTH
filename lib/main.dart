...
Future<void> main() async {
  ...
  // O StreamBuilder do AuthGate trata da espera pela sessão de forma reativa
 ...
 List<Tarefa> tarefas = [];
  ...
  Future<void> _carregarDados() async {
    ...
    try {
      ...
      if (mounted) setState(() => _diagStep = '⏳ A carregar config...');
      try {
        ...
        if (mounted) setState(() => _diagStep = '✅ Config carregada');
      } catch (e) {
        debugPrint('⚠️ Erro ao carregar config: $e');
      }

      // Perfil
      if (mounted) setState(() => _diagStep = '⏳ A carregar perfil...');
      try {
        ...
        if (perfilSnap.exists) {
          ...
          if (mounted) setState(() => _diagStep = '✅ Perfil carregado');
        } else {
      ...
      // Tarefas
      if (mounted) setState(() => _diagStep = '⏳ A carregar tarefas...');
      try {
        ...
        tarefas = tarefasSnap.docs
            ...
            .toList();
        if (mounted) setState(() => _diagStep = '✅ Tarefas: ${tarefas.length}');
      } catch (e) {
      ...
      // Sessões
      if (mounted) setState(() => _diagStep = '⏳ A carregar sessões...');
      try {
        ...
        sessoes = sessoesSnap.docs
            ...
            .toList();
        if (mounted) setState(() => _diagStep = '✅ Sessões: ${sessoes.length}');
      } catch (e) {
        debugPrint('⚠️ Erro ao carregar sessões: $e');
      }

      // Todo blocos
      if (mounted) setState(() => _diagStep = '⏳ A carregar TODO...');
      try {
        ...
        if (todoSnap.docs.isNotEmpty) {
          ...
          if (mounted) setState(() => _diagStep = '✅ TODO carregado');
        }
      } catch (e) {
      ...
      if (mounted) setState(() => _diagStep = '⏳ A restaurar timer...');
      try {
        final configSnap = await _configDoc.get().timeout(
          const Duration(seconds: 5),
          onTimeout: () {
        ...
        if (configSnap.exists) {
          ...
          if (mounted) setState(() => _diagStep = '✅ Timer restaurado');
        }
      } catch (e) {
      ...
      if (mounted) setState(() => _diagStep = '⏳ A migrar username...');
      try {
        await _migrarUsernameParaIndice();
        if (mounted) setState(() => _diagStep = '✅ Username migrado');
      } catch (e) {
