...
Future<void> _agendarNotificacaoDiaria() async {
  ...
  try {
    ...
    debugPrint('⚠️ zonedSchedule falhou (sem permissão de alarme exato?): $e');
  }
}
...
Future<void> main() async {
  ...
  // Notificações em background para não atrasar o arranque
  _inicializarNotificacoes().catchError((e) => debugPrint('⚠️ Notificações: $e'));
  ...
  Future<void> _carregarDados() async {
    ...
    Future.delayed(const Duration(seconds: 20), () {
      if (mounted && _aCarregarDados) {
        debugPrint('⚠️ Timeout de segurança: forçar fim do carregamento');
