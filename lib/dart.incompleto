  ...
  Timer.periodic(const Duration(seconds: 1), (timer) {
    if (service is AndroidServiceInstance) {
      service.setForegroundNotificationInfo(
        title: 'THOTH – Timer a correr',
        content: 'O temporizador Pomodoro está activo em background.',
      );
    }
    service.invoke('tick');
  });
...
Future<void> _iniciarBackgroundService() async {
  ...
  await service.configure(
    androidConfiguration: AndroidConfiguration(
      ...
      notificationChannelId: 'thoth_pomodoro_channel',
      initialNotificationTitle: 'THOTH – Timer a correr',
      initialNotificationContent: 'Pomodoro activo em background',
...
Future<void> main() async {
  ...
  if (!_isDesktop && !kIsWeb) {
    await _iniciarBackgroundService();
  }
...
class PerfilUsuario {
  ...
  factory PerfilUsuario.fromJson(Map<String, dynamic> j) =>
      PerfilUsuario(
        nome: j['nome'] as String? ?? '',
...
class StreakInfo {
  ...
  const StreakInfo(
      {required this.dias, this.ultimoEstudo, this.frozenHoje = false});
...
Color _tc() => temaEscuro.value ? Colors.white : Colors.black87;
  ...
  Future<void> signInWithGoogle() async {
    ...
    try {
      ...
      if (userCredential.user != null) {
        ...
        debugPrint(
            '✅ Login bem-sucedido para ${userCredential.user!.email}');
      }
    } catch (e) {
...
class _PomodoroAppState extends State<PomodoroApp>
  ...
  // ── NOVO: controlo do bloqueio de ecrã ──────────────────────────────
  // true  → o overlay de bloqueio está visível (foco a correr, não pausado)
  // false → overlay oculto
  bool _ecraBloquado = false;

  bool _aCarregarDados = true;
  String _diagStep = 'A iniciar...';
  ...
  Future<void> _restaurarTimerSeAtivo(
    ...
    setState(() {
      ...
      _primeiraVez = estava;
      // Restaurar estado do bloqueio: se estava a correr, bloquear
      _ecraBloquado = !estava && !noDesc;
    });
  ...
  Future<void> _carregarDados() async {
    ...
    try {
      ...
      if (mounted) setState(() => _diagStep = 'Utilizador: ${user.email}');

      if (mounted) setState(() => _diagStep = '⏳ Config...');
      try {
        final configSnap = await _configDoc
            .get()
            .timeout(const Duration(seconds: 5),
                onTimeout: () =>
                    throw TimeoutException('Timeout config'));
        if (configSnap.exists) {
          ...
          if (d['countdownAlvo'] != null) {
            _countdownAlvoGuardado =
                DateTime.tryParse(d['countdownAlvo'] as String);
          }
          if (d['countdownMotivo'] != null) {
            _countdownMotivoGuardado =
                d['countdownMotivo'] as String;
          }
      ...
      if (mounted) setState(() => _diagStep = '⏳ Perfil...');
      try {
        final perfilSnap = await _perfilDoc
            .get()
            .timeout(const Duration(seconds: 5),
                onTimeout: () =>
                    throw TimeoutException('Timeout perfil'));
        if (perfilSnap.exists) {
          perfil = PerfilUsuario.fromJson(
              perfilSnap.data() as Map<String, dynamic>);
        }
      } catch (e) {
        debugPrint('⚠️ Perfil: $e');
      }

      if (mounted) setState(() => _diagStep = '⏳ Tarefas...');
      try {
        final tarefasSnap = await _tarefasCol
            .orderBy('ordem')
            .get()
            .timeout(const Duration(seconds: 5),
                onTimeout: () =>
                    throw TimeoutException('Timeout tarefas'));
        tarefas = tarefasSnap.docs
            .map((d) =>
                Tarefa.fromJson(d.data() as Map<String, dynamic>))
            .toList();
      } catch (e) {
        debugPrint('⚠️ Tarefas: $e');
      }

      if (mounted) setState(() => _diagStep = '⏳ Última tarefa...');
      try {
        final configSnap = await _configDoc
            .get()
            .timeout(const Duration(seconds: 5),
                onTimeout: () =>
                    throw TimeoutException('Timeout ultima'));
        ...
        debugPrint('⚠️ Última tarefa: $e');
      }

      if (mounted) setState(() => _diagStep = '⏳ Sessões...');
      try {
        final sessoesSnap = await _sessoesCol
            ...
            .timeout(const Duration(seconds: 5),
                onTimeout: () =>
                    throw TimeoutException('Timeout sessões'));
        sessoes = sessoesSnap.docs
            .map((d) => SessaoConcluida.fromJson(
                d.data() as Map<String, dynamic>))
            .toList();
      } catch (e) {
        debugPrint('⚠️ Sessões: $e');
      }

      if (mounted) setState(() => _diagStep = '⏳ TODO...');
      try {
        final todoSnap = await _todoCol
            .get()
            .timeout(const Duration(seconds: 5),
                onTimeout: () =>
                    throw TimeoutException('Timeout todo'));
      ...
      if (mounted) setState(() => _diagStep = '⏳ Timer...');
      try {
        final configSnap = await _configDoc
            .get()
            .timeout(const Duration(seconds: 5),
                onTimeout: () =>
                    throw TimeoutException('Timeout timer'));
        if (configSnap.exists) {
          await _restaurarTimerSeAtivo(
              configSnap.data() as Map<String, dynamic>);
        }
      } catch (e) {
        debugPrint('⚠️ Timer: $e');
      }

      if (mounted) setState(() => _diagStep = '⏳ Username...');
      try {
        await _migrarUsernameParaIndice();
      } catch (e) {
        debugPrint('⚠️ Username: $e');
      }
  ...
  Future<void> _migrarUsernameParaIndice() async {
    try {
      ...
      debugPrint('Erro migrar username: $e');
    }
  }
  ...
  Future<void> _guardarTudo() async {
    ...
    batch.set(
        _perfilDoc, perfil.toJson(), SetOptions(merge: true));
  ...
  Future<void> _guardarTarefasImediato() async {
    ...
    try {
      ...
      debugPrint('Erro guardar tarefas: $e');
    }
  }
  ...
  void initState() {
    ...
    temaEscuro.addListener(() {
      _configDoc.set(
          {'temaEscuro': temaEscuro.value}, SetOptions(merge: true));
    });
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (isBackground) {
      ...
      if (estadoApp == EstadoApp.cronometro && !pausado) {
        _timerReferencia = DateTime.now();
        _segundosNaReferencia = segundosRestantes;
        _guardarEstadoTimer();
      }
    }
    ...
    if (state == AppLifecycleState.resumed) {
      if (estadoApp == EstadoApp.cronometro && !pausado) {
        if (_timerReferencia != null &&
            _segundosNaReferencia != null) {
          final elapsed = DateTime.now()
              .difference(_timerReferencia!)
              .inSeconds;
          final novosSegundos =
              (_segundosNaReferencia! - elapsed).clamp(0, 999999);
          if (mounted) {
            setState(() => segundosRestantes = novosSegundos);
            if (novosSegundos <= 0) _avancarFase();
          }
          _timerReferencia = null;
          _segundosNaReferencia = null;
        } else {
  ...
  void iniciarTarefa(Tarefa t, {bool retomar = false}) {
    ...
    setState(() {
      ...
      pausado = true;
      _primeiraVez = true;
      // Bloqueio começa fechado; abre quando o utilizador prime play
      _ecraBloquado = false;
  ...
  double _calcularProgressoGlobal() {
    ...
    final totalFase =
        estaNoDescanso ? tarefaAtual!.descanso : tarefaAtual!.estudo;
    final progressoFase = totalFase <= 0
        ? 0.0
        : (1 - (segundosRestantes / totalFase));
    final global =
        ((cicloAtual - 1) + progressoFase) / tarefaAtual!.ciclos;
  ...
  void _tocarSom({bool fim = false}) {
    try {
      ...
      final asset =
          fim ? 'sounds/timer_fim.wav' : 'sounds/timer_fase.wav';
  ...
  void _avancarFase() {
    ...
    if (!estaNoDescanso) {
      ...
      setState(() {
        ...
        // Ao entrar em descanso, desbloquear o ecrã
        _ecraBloquado = false;
        _salvarEstadoAtual();
      });
    } else {
      ...
      setState(() {
        ...
        // Ao começar novo ciclo de foco, bloquear se não pausado
        _ecraBloquado = !pausado;
        _salvarEstadoAtual();
      });
    }
  }
  ...
  /// Alterna pausa/play e actualiza o estado do bloqueio de ecrã.
  void alternarPausa() {
    setState(() {
      pausado = !pausado;
      _primeiraVez = false;
      // Bloquear apenas durante foco activo (não durante descanso)
      if (!pausado && !estaNoDescanso) {
        _ecraBloquado = true;
      } else {
        _ecraBloquado = false;
      }
  ...
  /// Desbloqueia o ecrã e pausa o timer (chamado pelo overlay).
  void desbloquearEcra() {
    setState(() {
      pausado = true;
      _ecraBloquado = false;
    });
  ...
  void descartarProgresso() {
    setState(() {
      if (ultimaTarefa != null) {
        ...
        final data = t.toJson();
        final idx =
            tarefas.indexWhere((x) => x.id == t.id);
        if (idx >= 0) data['ordem'] = idx;
  ...
  void finalizar() {
    ...
    setState(() {
      estadoApp = EstadoApp.fim;
      _ecraBloquado = false; // garantir desbloqueio ao finalizar
    });
    _guardarTudo();
  ...
  void reset() {
    ...
    setState(() {
      ...
      tarefaAtual = null;
      pausado = false;
      _ecraBloquado = false; // garantir desbloqueio ao resetar
    });
    _configDoc.set({
      'timerAtivo': false,
      'timerReferencia': null,
  ...
  Widget build(BuildContext context) {
    ...
    try {
      // ── NOVO: se o ecrã está bloqueado, mostrar overlay por cima de tudo
      if (_ecraBloquado && tarefaAtual != null) {
        return _EcraFocoBloqueado(state: this);
      }
...
// =============================================================================
// ECRÃ DE BLOQUEIO DURANTE FOCO  ← NOVO
// =============================================================================

class _EcraFocoBloqueado extends StatefulWidget {
  final _PomodoroAppState state;
  const _EcraFocoBloqueado({required this.state});

  @override
  State<_EcraFocoBloqueado> createState() =>
      _EcraFocoBloqueadoState();
}

class _EcraFocoBloqueadoState extends State<_EcraFocoBloqueado>
    with SingleTickerProviderStateMixin {
  late AnimationController _pulseCtrl;
  late Animation<double> _pulse;

  // Para o gesto de deslizar para desbloquear
  ...
  void _tentarDesbloquear() {
    widget.state.desbloquearEcra();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final total = tarefa.estudo;
    final progresso = total <= 0
        ? 0.0
        : (1 - (state.segundosRestantes / total)).clamp(0.0, 1.0);

    // Impede o botão voltar do Android
    return PopScope(
      canPop: false,
      child: Scaffold(
        backgroundColor: Colors.black,
        body: GestureDetector(
          // Deslizar para cima para mostrar o botão de desbloqueio
          onVerticalDragUpdate: (details) {
            setState(() {
              _swipeOffset =
                  (_swipeOffset - details.delta.dy).clamp(0.0, 200.0);
            });
          },
          onVerticalDragEnd: (_) {
          ...
          child: Stack(
            children: [
              // ── Fundo com gradiente radial pulsante ─────────────
              AnimatedBuilder(
                animation: _pulse,
                builder: (_, __) => Container(
                  decoration: BoxDecoration(
                    gradient: RadialGradient(
                      ...
                      radius: _pulse.value * 1.3,
                      colors: [
                        azul.withOpacity(0.09 * _pulse.value),
                        Colors.black,
                      ],
                    ),
                  ),
              ...
              SafeArea(
                child: Column(
                  children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          // Ciclo actual
                          Text(
                            'Ciclo ${state.cicloAtual}/${tarefa.ciclos}',
                            style: const TextStyle(
                              color: Colors.white38,
                              fontSize: 12,
                              letterSpacing: 0.5,
                    ...
                    // ── Rodapé: instrução de desbloqueio ─────────
                    Padding(
                      padding: const EdgeInsets.only(bottom: 36),
                      child: AnimatedBuilder(
                        animation: _pulse,
                        builder: (_, __) {
                          // Mostrar botão de desbloqueio quando
                          // o utilizador já deslizou o suficiente
                          final showButton =
                              _swipeOffset > 40;
                          return Column(
                            children: [
          ...
          _drawerItem(
            ...
            onTap: () {
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (_) =>
                      TelaInsights(sessoes: state.sessoes),
                ),
              );
            },
          ),
          ...
          _drawerItem(
            ...
            onTap: () {
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (_) =>
                        _TelaTarefasConcluidas(state: state)),
              );
            },
          ),
          _drawerItem(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('OK',
                style:
                    TextStyle(color: Color(0xFF1D81C7))),
          ),
        ],
      ),
    );
            ...
            Row(
              children: [
                ...
                Text(
                  ...
                  style:
                      TextStyle(color: _tc54(), fontSize: 12),
                ),
              ],
            ),
            const SizedBox(height: 8),
            ...
            SizedBox(
              ...
              child: OutlinedButton.icon(
                ...
                label: const Text(
                  'Partilhar streak',
                  style:
                      TextStyle(color: Color(0xFFFF6D00)),
                ),
          ...
          Row(
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () =>
                      state.iniciarTarefa(ultimaTarefa,
                          retomar: true),
...
class _CartaoTarefa extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const azul = _PomodoroAppState._azul;
    final branco = _tc();
    final temProgresso = tarefa.progressoSalvo > 0.0 &&
      ...
      child: Container(
        ...
        child: ListTile(
          ...
          subtitle: Text(
            ...
            style:
                const TextStyle(color: azul, fontSize: 13),
          ),
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            _labelSlider(
                'Número de Ciclos', '${t.ciclos} ciclos'),
            Slider(
  ...
  Widget _labelSlider(String titulo, String valor) {
    return Row(
      ...
      children: [
        Text(titulo,
            style: TextStyle(
                color: _PomodoroAppState._branco
                    .withOpacity(0.7),
                fontSize: 13)),
        Text(valor,
            style: const TextStyle(
                color: _PomodoroAppState._azul,
                fontWeight: FontWeight.bold,
                fontSize: 14)),
      ],
    );
            ...
            children: [
              ...
              if (nome.isNotEmpty)
                Text(
                  nome,
                  style:
                      const TextStyle(color: azul, fontSize: 16),
                  textAlign: TextAlign.center,
                ),
              const SizedBox(height: 40),
              Row(
  ...
  return Column(
    children: [
      ...
      Text(
        label,
        style: TextStyle(
            color: _tc().withOpacity(0.5), fontSize: 12),
      ),
    ],
  );
}
      ...
      appBar: AppBar(
        ...
        actions: [
          _aCarregarFoto
              ...
              : IconButton(
                  icon:
                      const Icon(Icons.check, color: azul),
                  tooltip: 'Guardar',
                  onPressed: _guardar,
                ),
        ],
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          helperStyle: TextStyle(
              ...
              fontSize: 11),
          hintStyle:
              TextStyle(color: _tc().withOpacity(0.3)),
...
class TelaInsights extends StatelessWidget {
  ...
  const TelaInsights(
      {super.key, required this.sessoes});
          ...
          body: ListView(
            ...
            children: [
              ...
              _tile(
                  ...
                  onTap: () {},
                  readOnly: true),
              _tile(Icons.mail_outline, 'Email',
                  _email, fg, fgMuted, tileBg, border,
                  onTap: () {}, readOnly: true),
              _tileToggle(
                  Icons.lock_outline,
                  'Privacidade',
...
class _TelaAmigosState extends State<TelaAmigos> {
  ...
  Set<String> _pedidosEnviados = {}; // UIDs para quem já enviei pedido pendente
  bool _loading = true;
  final TextEditingController _searchCtrl = TextEditingController();
  List<Map<String, dynamic>> _resultados = [];
  bool _searching = false;
  StreamSubscription<QuerySnapshot>? _pedidosSub;
  StreamSubscription<QuerySnapshot>? _pedidosEnviadosSub;
  ...
  void initState() {
    ...
    // Stream real-time para pedidos ENVIADOS pelo utilizador
    _pedidosEnviadosSub = _db
        .collection('pedidos_amizade')
        .where('de', isEqualTo: widget.uid)
        .snapshots()
        .listen((snap) {
      final enviados = snap.docs
          .where((d) => (d.data()['estado'] as String? ?? '') == 'pendente')
          .map((d) => d.data()['para'] as String? ?? '')
          .where((uid) => uid.isNotEmpty)
          .toSet();
      if (mounted) {
        setState(() {
          _pedidosEnviados = enviados;
          // Actualizar resultados de pesquisa visíveis
          for (final r in _resultados) {
            r['jaPedido'] = enviados.contains(r['uid'] as String? ?? '');
          }
        });
      }
    });
      ...
      if (usernameDoc.exists) {
        ...
        if (uid.isNotEmpty && uid != widget.uid) {
          ...
          if (perfilSnap.exists) {
            ...
            final jaPedido = _pedidosEnviados.contains(uid);
